...
<CFLOOP query="GetPowPts">
	...
	SET PowerPtsAdj = #GetPowPts.PowerPtsAdj#
...
<cfloop query="GetTeams">
	...
	SELECT Team,Opp,week,ha,pswogarbage as ps,dpswogarbage as dps,pavg,dpavg
	FROM Sysstats
	...
	<cfloop query="GetOpp">
			...
			SELECT PowerPtsAdj,PowerPassPts
			FROM BetterThanAvg
			...
			<cfif Getopp.DPS lte GetOpp.PS>
				...
				<cfif GetOpp.PS - GetOpp.dPS lte 3>
					<cfset pts = OppPowPts.PowerPtsAdj + 2>
				<cfelseif GetOpp.PS - GetOpp.dPS lte 6>
					<cfset pts = OppPowPts.PowerPtsAdj + 5>
				<cfelseif GetOpp.PS - GetOpp.dPS lte 9>
					<cfset pts = OppPowPts.PowerPtsAdj + 8>
				<cfelse>
					<cfset pts = OppPowPts.PowerPtsAdj + 11>
				</cfif>
	...
	<cfquery name="UpdPowPts" datasource="sysstats3">
		UPDATE BetterThanAvg
		SET PerformancePts = #TotPts/gamect#
		WHERE TEAM = '#GetTeams.Team#'
	</cfquery>
...
<cfquery name="UpdPowPts" datasource="sysstats3">
		UPDATE BetterThanAvg
		SET PowerPtsAdj = PerformancePts
		WHERE TEAM = '#GetTeams.Team#'
</cfquery>
