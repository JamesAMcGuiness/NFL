...
<cfset temp = updateNFLSPDSFlags(19)>
<cfset temp = createPreGameStats(19)>
<cfabort>
...
<cfargument name="team" type="String"  required="no" />
	...
	<cfif isdefined("arguments.team")>
		<cfquery datasource="psp_psp" name="GetInfo">
...
<cfargument name="theWeek"    type="Numeric"  required="yes" />
		...
		<cfquery datasource="psp_psp" name="GetGames2">
		SELECT MAX(Gametime) as MNF, MIN(Gametime) as TNF 
		FROM NFLSPDS
		where week = #arguments.theweek#
		</cfquery>

		<cfquery datasource="psp_psp" name="GetUpd">
		UPDATE NFLSPDS
		SET MNF_FLAG   = 'Y'
		...
		<cfquery datasource="psp_psp" name="GetTmsMNF">
		SELECT Fav, Und 
		FROM NFLSPDS
		where Gametime = '#GetGames2.MNF#'
		</cfquery>

		<cfquery datasource="psp_psp" name="GetTmsTNF">
		SELECT Fav, Und 
		FROM NFLSPDS
...
<cfargument name="theWeek"    type="Numeric"  required="yes" />
	...
	<cfset vUndHFANum = 0>
		...
		<cfset vFavPowerRating     = #agi.PowerRatingAdjForOppWithTrending#>	
		...
		<cfset vUndPowerRating     = #agi.PowerRatingAdjForOppWithTrending#>	
	...
	*************************** #hfa#<br>
	...
	<td bgcolor="#ExtraTravelColor#">#AdvExtraTravel#</td>
